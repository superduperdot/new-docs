...
| `GET /…`, `DELETE /…` | Not required          |

If the header is missing on a POST or PATCH request, Brale returns `400 Bad Request`:
...
<Note>
The error message mentions "POST requests" but the `Idempotency-Key` header is also required on `PATCH` endpoints (e.g., updating an address).
</Note>
