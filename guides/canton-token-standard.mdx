...
<Note>
The curl examples below call the **Canton Ledger API** (not the Brale API). The `Accept: application/json` header is correct for the Canton JSON API. For Brale API requests, omit the Accept header — see [Accept Header](/api-reference/brale-introduction#accept-header).
</Note>
